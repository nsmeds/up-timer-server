...
const apiKey = process.env.UPTIMER;
    ...
    return request.post('https://api.uptimerobot.com/v2/getMonitors')
        ...
        .then( res => {
            if (!res.body.monitors) {
                if (res.code === 'ECONNREFUSED') throw new Error('Connection error; please try again');
...
app.pauseAll = () => {
    app.getMonitors()
        ...
        .then(keys => keys.map(key => app.pause(key)))
};

app.resumeAll = () => {
    app.getMonitors()
        .then(res => Object.keys(res))
        .then(keys => keys.map(key => app.resume(key)))
};
